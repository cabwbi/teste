(function(){
  ...
  function labelFor(sel){const text=sel.closest('label')?.querySelector('span')?.textContent||'opções'; return 'Todas as '+text.toLowerCase().replace('ug requisitante','UGs').replace('ação orçamentária','ações').replace('natureza de despesa','naturezas').replace('empresa contratada','empresas').replace('requisição atrasada','situações');}
  ...
  function fill(id, vals){const sel=$(id); if(!sel)return; const old=new Set(selected(sel)); sel.innerHTML=vals.map(v=>'<option value="'+esc(v)+'">'+esc(v)+'</option>').join(''); Array.from(sel.options).forEach(o=>o.selected=old.has(o.value)); enhance(sel); sel.onchange=render;}
  function filters(){return {ug:selected($('#rpUg')),acao:selected($('#rpAcao')),nat:selected($('#rpNatureza')),proj:selected($('#rpProjeto')),emp:selected($('#rpEmpresa')),atrasada:selected($('#rpAtrasada'))};}
  function filtered(){const f=filters(); return records.filter(r=>(!f.ug.length||f.ug.includes(r.ug))&&(!f.acao.length||f.acao.includes(r.acao))&&(!f.nat.length||f.nat.includes(r.natureza))&&(!f.proj.length||f.proj.includes(r.projeto)||f.proj.includes(r.projetosReq))&&(!f.emp.length||f.emp.includes(r.empresa))&&(!f.atrasada.length||f.atrasada.includes('TODAS')||f.atrasada.includes(r.requisicaoAtrasada)));}
