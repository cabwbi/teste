...
function renderExecutive(){ if(!$('#kpiCreditAvailable')) return; const {digits,pos,sig}=filtered(); const credito=digits.reduce((a,r)=>a+Number(r.saldo||0),0); const empenhado=pos.reduce((a,r)=>a+Number(r.valorUsd||0),0); const assinatura=sig.reduce((a,r)=>a+Number(r.valorUsd||0),0); const total=credito+empenhado+assinatura; const faturadoPo=pos.reduce((a,r)=>a+Number(r.faturadoUsd||0),0); const saldoPo=pos.reduce((a,r)=>a+Number(r.saldoUsd||0),0); const fornecedores=unique(pos.map(r=>r.fornecedor||r.nomeFornecedor)); $('#kpiCreditAvailable').textContent=money(credito); $('#kpiCommitted').textContent=money(empenhado); $('#kpiSigning').textContent=money(assinatura); $('#kpiReceived').textContent=money(total); $('#kpiAvailablePct').textContent=pct(credito,total); $('#kpiDigitsCount').textContent=num(digits.length); $('#kpiPoCount')&&($('#kpiPoCount').textContent=num(pos.length)); $('#kpiSuppliersCount')&&($('#kpiSuppliersCount').textContent=num(fornecedores.length)); $('#kpiPoBilled')&&($('#kpiPoBilled').textContent=money(faturadoPo)); $('#kpiPoBalance')&&($('#kpiPoBalance').textContent=money(saldoPo)); const table=$('#executiveSummaryBody'); if(table)table.innerHTML=`<tr><td>2026</td><td class="text-right">${money(credito)}</td><td class="text-right">${money(empenhado)}</td><td class="text-right">${money(assinatura)}</td><td class="text-right">${money(total)}</td><td class="text-center">${pct(credito,total)}</td><td class="text-center">${num(digits.length)}</td></tr>`; drawPie('creditByGcChart', agg(digits, r=>r.gcDescricao||r.grandComando||'N/I', r=>r.saldo), 'Crédito disponível por Grande Comando'); drawPie('poByGcChart', agg(pos, gcFromDigit, r=>r.valorUsd), 'PO por Grande Comando conforme dígito'); drawBar('poByCompanyChart', agg(pos, r=>r.fornecedor||'N/I', r=>r.valorUsd), 'Empenhos por empresa', 'Valor empenhado'); drawBar('poByProjectChart', agg(pos, r=>projectLabels(r).join(', ')||'Sem projeto', r=>r.valorUsd), 'Empenhos por projeto', 'Valor empenhado'); renderDigitsTable(digits); }
